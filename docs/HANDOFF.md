...
A full-source review on 2026-09-21 is recorded in [`docs/reviews/2026-09-21-walking-skeleton-control-plane.md`](reviews/2026-09-21-walking-skeleton-control-plane.md). Three defects are demonstrated rather than inferred, and all three are open: `Spex._pipe_monitor` calls `call_from_thread` outside the app's running window and loses Hub-loss reporting; the Hub telemetry reporter thread outlives `reporter.cancel()` and sends on a closed pipe; and `ServiceProcess` tests its shutdown flag only between work cycles, which fails once cycles are long-lived. Work cycles are long-lived by design, so a worker must observe shutdown from inside `_run_cycle`; the base loop's test is a backstop. Its findings and open questions are tracked in `docs/TODO.md`.

The earlier Hub-only review findings are resolved. `_join_service`'s blocking terminate/kill escalation runs through `asyncio.to_thread`, and `_join` overlaps every child's escalation under one `asyncio.gather`. `run()` owns supervision on one event loop; signal handlers only record shutdown intent, and the Hub context completes child cleanup before releasing the lock.
...
- Textual 8.2.8 raises `RuntimeError` from `call_from_thread` both before `App.run()` (`App._loop` is `None`) and after it returns (`_loop` holds a closed loop, never reset to `None`).
- Driving `IngestionService._run_cycle` directly measures 0.250 seconds per cycle against a 0.100-second sleep budget; the excess is the snapshot-thread join and amortizes away once cycles are long-lived.
- Pydantic 2.13.4 accepts `model_validate(..., extra="forbid")` and `model_dump(exclude_computed_fields=True)`; `config.py` uses both correctly.
