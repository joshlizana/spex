...
One open question awaiting Joshua's decision: several remaining `REFACTOR_TODO.md` items are implementation rather than refactor mechanics, and three of them duplicate `docs/TODO.md` 0.2 lines 35, 42, and 43 verbatim — step 9's operator intents, background-worker state receipt, and real health indicator, plus its terminal-ownership item and step 12's bidirectional-message confirmation. The proposal is to delete them from the refactor checklist, leave a pointer to 0.2, and keep the refactor scoped to the supervision loop (step 8), the TUI pipe wiring (step 9), the entry point (step 10), and step 12's mechanical confirmations. Not yet decided.
