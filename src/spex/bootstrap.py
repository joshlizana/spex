...
def bootstrap_spex() -> None:
    ...
    for directory in [config.data_dir, config.log_dir, config.config_dir, config.cache_dir, config.runtime_dir, config.state_dir]:
        directory.mkdir(parents=True, exist_ok=True)

    # Create durable application-data boundaries.
    for subdirectory in ["raw", "raw/live", "raw/backfill", "ducklake", "rejected", "credentials"]:
    ...
    # Create process-lock and IPC boundaries.
    for subdirectory in ["locks", "ipc"]:
        (config.runtime_dir / subdirectory).mkdir(parents=True, exist_ok=True)
